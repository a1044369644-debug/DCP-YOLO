...
# ----------------------------
# Dynamic Attenuation Mask
# ----------------------------
class DynamicAttenuationMask(nn.Module):
    """
    动态衰减掩码：
    通过 avg + max 的全局统计生成通道权重，
    用于抑制背景噪声、突出目标相关响应。
    """
    def __init__(self, c, reduction=8):
        super().__init__()
        c_ = max(8, c // reduction)
        self.conv1 = nn.Conv2d(2 * c, c_, 1, bias=False)
        self.act = nn.SiLU()
        self.conv2 = nn.Conv2d(c_, c, 1, bias=True)
...
class CrossAxisGate(nn.Module):
    """
    分轴建模：
    分别沿 H/W 两个方向提取响应，再融合成空间门控图。
    比完整 self-attention 更轻量。
    """
    def __init__(self, c, k=7):
        ...
        # 对 H 方向建模：输入 [B, C, H, 1]
        self.conv_h = nn.Conv2d(c, c, kernel_size=(k, 1), padding=(p, 0), groups=c, bias=False)

        # 对 W 方向建模：先转成 [B, C, W, 1] 再做同样卷积
        self.conv_w = nn.Conv2d(c, c, kernel_size=(k, 1), padding=(p, 0), groups=c, bias=False)

        self.fuse = nn.Conv2d(c, c, 1, bias=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # H-axis pooling
        x_h = x.mean(dim=3, keepdim=True)  # [B, C, H, 1]
        a_h = self.conv_h(x_h)             # [B, C, H, 1]

        # W-axis pooling
        x_w = x.mean(dim=2, keepdim=True).transpose(2, 3)  # [B, C, W, 1]
        a_w = self.conv_w(x_w).transpose(2, 3)             # [B, C, 1, W]

        # broadcast fusion
        a = a_h + a_w
        a = self.sigmoid(self.fuse(a))
...
# DGCA Bottleneck
# ----------------------------
class DGCABottleneck(nn.Module):
    """
    DGCA 核心瓶颈块：
    1) 通道混合
    2) 分成保真分支 + 增强分支
    3) 增强分支做局部卷积 + 动态衰减 + 分轴建模
    4) DSConv 重建
    5) 残差连接
    """
    def __init__(self, c, shortcut=True, local_k=3, axis_k=7):
        super().__init__()
        self.cv1 = Conv(c, c, 1, 1)

        self.c_keep = c // 2
        self.c_enh = c - self.c_keep

        # 增强分支
        self.local1 = Conv(self.c_enh, self.c_enh, local_k, 1)
        self.local2 = DSConv(self.c_enh, self.c_enh, 3, 1)

        self.mask = DynamicAttenuationMask(self.c_enh, reduction=8)
        self.axis_gate = CrossAxisGate(self.c_enh, k=axis_k)

        # 重建与融合
        self.rebuild = DSConv(self.c_enh * 2, self.c_enh, 3, 1)
    ...
    def forward(self, x):
        ...
        y1, y2 = torch.split(y, [self.c_keep, self.c_enh], dim=1)

        # 局部建模
        y2_local = self.local2(self.local1(y2))

        # 动态衰减
        attn_mask = self.mask(y2_local)
        y2_masked = y2_local * attn_mask

        # 分轴增强
        y2_axis = self.axis_gate(y2_masked)

        # 重建
        y2_out = self.rebuild(torch.cat([y2_local, y2_axis], dim=1))

        out = self.cv2(torch.cat([y1, y2_out], dim=1))
...
# DGCA-C3k2
# ----------------------------
class DGCA_C3k2(nn.Module):
    """
    可替换 C3k2 的版本
    写法尽量保持与 Ultralytics 的 C2f/C3k2 风格一致。
    """
    def __init__(self, c1, c2, n=1, shortcut=False, s=1, e=0.5, local_k=3, axis_k=7):
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        ...
        self.m = nn.ModuleList(
            DGCABottleneck(self.c, shortcut=shortcut, local_k=local_k, axis_k=axis_k)
            for _ in range(n)
        )
    ...
    def forward_split(self, x):
        y = list(torch.split(self.cv1(x), (self.c, self.c), 1))
        for m in self.m:
            y.append(m(y[-1]))
        return self.cv2(torch.cat(y, 1))