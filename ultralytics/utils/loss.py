...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        if class_weights is not None:
            ...
            if class_weights.numel() != self.nc:
                raise ValueError(f"class_weights length {class_weights.numel()} must match nc={self.nc}")
            self.class_weights = class_weights.view(1, 1, self.nc)
        else:
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        if self.class_weights is not None:
            positive_weight = 1 + target_scores.gt(0).to(dtype) * (self.class_weights.to(dtype=dtype) - 1)
            cls_loss *= positive_weight
