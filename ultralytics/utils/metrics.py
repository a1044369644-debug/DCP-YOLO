...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    def __init__(self, names: dict[int, str] = {}) -> None:
        ...
        self.size_stats = {k: dict(tp=[], conf=[], pred_cls=[], target_cls=[]) for k in self.size_names}
        self.size_maps = dict.fromkeys(self.size_names, 0.0)
    ...
    def update_size_stats(self, size: str, stat: dict[str, Any]) -> None:
        """Update small/medium/large object statistics for mAP calculation."""
        for k in self.size_stats[size]:
            self.size_stats[size][k].append(stat[k])
    ...
    def process(self, save_dir: Path = Path("."), plot: bool = False, on_plot=None) -> dict[str, np.ndarray]:
        ...
        self.process_size_stats()
        return stats

    def process_size_stats(self) -> None:
        """Process small/medium/large object mAP from accumulated validation stats."""
        self.size_maps = dict.fromkeys(self.size_names, 0.0)
        for size, size_stats in self.size_stats.items():
            if not size_stats["tp"]:
                continue
            stats = {k: np.concatenate(v, 0) for k, v in size_stats.items()}
            if not stats["target_cls"].size:
                continue
            ap = ap_per_class(stats["tp"], stats["conf"], stats["pred_cls"], stats["target_cls"])[5]
            self.size_maps[size] = float(ap.mean()) if len(ap) else 0.0

    def clear_stats(self):
        """Clear the stored statistics."""
        for v in self.stats.values():
            v.clear()
        for size_stats in self.size_stats.values():
            for v in size_stats.values():
                v.clear()
    ...
    def size_results(self) -> list[float]:
        """Return mAP50-95 for small, medium, and large objects."""
        return [self.size_maps[k] for k in self.size_names]
    ...
    def results_dict(self) -> dict[str, float]:
        ...
        if self.task == "detect":
            results.update(
                {
                    "metrics/mAP(S)": self.size_maps["small"],
                    "metrics/mAP(M)": self.size_maps["medium"],
                    "metrics/mAP(L)": self.size_maps["large"],
                }
            )
        return results
