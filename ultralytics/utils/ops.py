...
def non_max_suppression(
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if not n:  # no boxes
            continue
        # For OBB tasks, reduce max_nms to prevent OOM in batch_probiou (which creates N×N matrix)
        # batch_probiou memory usage is O(N²), so we need stricter limits for rotated boxes
        effective_max_nms = max_nms if not rotated else min(max_nms, 10000)
        if n > effective_max_nms:  # excess boxes
            filt = x[:, 4].argsort(descending=True)[:effective_max_nms]  # sort by confidence and remove excess boxes
            x, xk = x[filt], xk[filt]
